	...
	<div class="home">
		...
		<el-form style="width:25rem;margin:auto" :label-position="labelPosition" label-width="220px" :model="formLabelAlign">
			...
			<el-form-item label="单位名称" class="apply">
				<el-input v-model="formLabelAlign.unname" :disabled="true"></el-input>
			</el-form-item>
			...
			<el-form-item label="创新券申领金额">
				<el-input style="width:6.5rem" v-model="formLabelAlign.applyMoney"></el-input><span class="yuan">元</span>
			</el-form-item>
...
<script>
	...
	import { InnovatApply_T, searchServiceName ,getUserInfo , post_uploadImg,appliMoneyLimit} from 'src/model/api'
...
//				imageUrl:'',
		...
		methods: {
			...
			//提交申请操作
			async InnovatApply() {
				              if(this.formLabelAlign.accountType.trim() == '') {
				              ...
				              console.log(this.appliMoneys)
				              
				              
				              if(this.formLabelAlign.applyMoney > this.appliMoneys){
				              	 util.errortip('您的企业已申领的科技创新券总额已超过市科技局规定的申领总额。请您减少申领金额！');
				              	 
				              	 if(this.formLabelAlign.applyMoney > (this.formLabelAlign.contractValue)/2){
				              	 	util.errortip('您的当前合同名称申领的科技创新券总额已超过合同金额的50% 。请您减少申领金额');
				              	 	return;
				              	 }
				              }
				              if(this.formLabelAlign.applyMoney.trim() == '') {
				                  util.errortip('申领金额不能为空!');
				                  return;
				              }
				              var regAM = /^[1-9]\d*$/
				              if(!regAM.test(this.formLabelAlign.applyMoney)) {
							...
							obj.entname = this.formLabelAlign.unname;
							...
							obj.appliMoney = this.formLabelAlign.applyMoney;
							console.log(obj)
							var res = await InnovatApply_T(obj);
							console.log(res)
							if(res.body.code == 0) {
						...
						async getUserInfo_info(){
							...
							this.formLabelAlign.unname = res.body.data.unitName;
							console.log(this.formLabelAlign.unname)
						},
						//机构名称数据检索
		...
		mounted() {
			...
			//获取cookie信息 usertype
			var x = util.handleCookieGet("userType");
			if(x.userType ==1){this.formLabelAlign.accountType = "企业"}
			if(x.userType ==2){this.formLabelAlign.accountType = "创新创业团队"}
			if(x.userType ==3){this.formLabelAlign.accountType = "区县用户"}
			if(x.userType ==4){this.formLabelAlign.accountType = "市科技局用户"}
		}
